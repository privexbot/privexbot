...
def upgrade() -> None:
    ...
    # Only create tables if they don't already exist (handles duplicate migrations)
    if not table_exists('kb_analytics_events'):
    ...
    if not table_exists('kb_audit_logs'):
    ...
    if not table_exists('kb_members'):
    ...
    if not table_exists('kb_notifications'):
    ...
    # Column additions with safety checks
    if not column_exists('chunks', 'kb_id'):
