...
async def verify_signature(
    ...
    if auth_identity:
        ...
        # Check if this Cosmos address is in the initial staff list
        is_initial_staff = should_be_staff("cosmos", address)
        ...
        try:
            ...
            # Concurrent first-time signin race — recover by re-fetching
            # the now-existing AuthIdentity and treating as sign-in.
            db.rollback()
            auth_identity = db.query(AuthIdentity).filter(
                AuthIdentity.provider == "cosmos",
                AuthIdentity.provider_id == address,
