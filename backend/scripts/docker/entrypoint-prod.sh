...
# `"secret-ai-v1"` placeholder to whatever `Secret().get_models()[0]` is
# now. Idempotent (re-runs after the first report zero rows). Non-blocking
# so a transient Secret AI outage at boot does not keep the API down —
# the runtime coercion in secret_ai_sdk_provider._get_client_for_model
# already handles any unmigrated rows on the next chat request.
echo ""
