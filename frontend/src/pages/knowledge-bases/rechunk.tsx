...
import { ArrowLeft, RefreshCw, Play, AlertCircle, Info, Zap, Eye } from 'lucide-react';
...
export default function KBRechunkPage() {
  ...
  const handlePreviewRechunk = async () => {
    ...
    try {
      const preview = await kbClient.kb.previewRechunk(kbId, kb.chunking_config);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: 'Knowledge base has been reindexed with new chunking configuration',
        });
            ...
            {kb && kb.chunking_config ? (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <Badge variant="outline">{kb.chunking_config.strategy || 'Not set'}</Badge>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Chunk Size</div>
                  <div className="font-medium">{kb.chunking_config.chunk_size || 'Not set'} chars</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Overlap</div>
                  <div className="font-medium">{kb.chunking_config.chunk_overlap || 'Not set'} chars</div>
                </div>
                <div>
                  <div className="text-sm text-muted-foreground">Total Chunks</div>
                  <div className="font-medium">{kb.stats?.chunks || 0}</div>
                </div>
              </div>
            ) : (
              <div className="text-muted-foreground">
                No chunking configuration available
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Adjust chunking parameters and preview the impact
                </CardDescription>
              </CardHeader>
              <CardContent>
                {kb && <KBChunkingConfig />}
            ...
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                Rechunking will regenerate all chunks and embeddings. This process may take several minutes
                ...
                {previewData ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <div className="text-sm text-muted-foreground">New Chunks</div>
                        <div className="text-2xl font-bold">{previewData.total_chunks || 0}</div>
                      </div>
                      <div>
                        ...
                        {previewData.sample_chunks.slice(0, 3).map((chunk: any, index: number) => (
                          <div key={index} className="p-3 bg-muted rounded-lg">
                            <div className="text-xs text-muted-foreground mb-2">
                              Chunk {index + 1} • {chunk.length || chunk.content?.length || 0} chars
                            </div>
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p>Click "Preview Changes" to see how your content will be chunked</p>
                  </div>
                )}
              </CardContent>
            </Card>
